...
**当前状态：这条路线已经跑通了，而且已经在真机上用起来了。**
代码在 `main` 上，云端 macOS 编译**跑过 49 次、成功 43 次**、连续 11 次成功，
产出 `HerInfo.app`（含内嵌的两个扩展）以及一份**可侧载的 IPA**。

> **这三个数是一个快照 —— 截至 `bdd7a6b`（run #49）。别把它当现状读**：
> 以后每次推送都会 ＋1，而且**写文档的这个提交自己也会跑一次**。
> 要现成的数，看 Actions 页面，或 `GET /repos/haohao234/She/actions/runs`。
> **一句会被推翻的话比一个过时的数字更坏 —— 它让人不再去查**（下面第 36~38 轮就是例子）。
**2026-09-11：这份 IPA 已在 iPhone（iOS 17.7）上安装成功并可以正常使用** ——
即「没有 Mac 也能把这个 App 装到手机上用」这条路，从编译到装机全线打通。

> **装机这一段花了三轮才通，值得留个记录：**
> 前两次失败都报 `bundleVersion must be set in placeholder attributes for an
> app extension placeholder`。查实的结论**不是签名问题、不是 Apple ID、不是网络** ——
> 是**装在了修复前的旧 IPA 上**（旧包三个 bundle 都缺 `CFBundleVersion`，
> 因为 `GENERATE_INFOPLIST_FILE` 盖掉了手写的版本键）。
> 而「用户会拿到旧包」是**结构性**的：artifact 留 30 天、新旧包在同一页里
> 只差文件名里 7 位提交号。处置与判据在 `她的信息本-iOS编译操作指引.html` STEP 05
> 的「先认包」和「万一失败」两处 —— 一句话：**看 Sideloadly 第一行
> `Using IPA file:`，确认拖的是新包（约 1.15 MB）而不是旧的（约 2 MB）。**
...
| 第 7–32 轮 | **全部成功** | 预检清单第 6 节 · 引导屏 + 轻提示 + 删除二次确认 + 回收站 + 首条记录引导 · 推送脚本 v4/v5 · **07 历史版本左右对比** · **04/30 搜索与筛选** · **08 键盘工具栏** · **09/10 情绪打标与即时建议** · **09 系统级入口（长按图标 / 快捷指令）** · **分享扩展** · **可侧载 IPA** · **Release 打包 + 拆包自检（扩展数量 / 版本键 / 调试动态库）**。<br>截至 `6992d38`（第 32 轮）已连续 **29 次成功**（按 CI 实际运行次数计）。<br>但**「成功」不等于「干净」**：第 12~25 轮（连续 14 轮）每一轮都带着同一条 warning 而没人发现，现在它是会红的断言 |
| 第 33–35 轮 | **成功** | 真机安装成功、App 可正常使用 · 功能汇总文档 + 修掉两个「假开关」（应用锁 / 导出验身）· 第一版 App 图标（人像） |
| **第 36–38 轮** | ⚠️ **3 个错（连红三次）** | 改名 + 问候语 + **生理期功能**一并推上去 → `CycleView.swift:147:22: error: cannot find 'shortDay' in scope` —— **`statRow` 调了一个根本没写的函数**。第 36 轮 `compile step exit code 65` 却**看不到原因**（job 日志要 admin、artifact 里的 `build.log` 要认证）；第 37 轮换报错出口仍没解；**第 38 轮先让 CI 把错误 `echo "::error::…"` 发成 annotation，才第一次看到那行行号**。这三次红真正的产出是一条基础设施：**红了要能看到原因** |
| 第 39–49 轮 | **全部成功（11 次）** | 修 `shortDay` 并把统计改成吃值不吃对象 · 补齐校验 · **App 图标定稿（银渐层英短 + 一颗心）** · **修「导入图片时闪退」**（48MP 原图懒解码 ≈195MB）· **新建记录时也能加图片** + 唯一 id 熵 24→48 bit · 文档同步到十份校验（第 44 轮是 `9525c82` 的**手动重跑**，所以它与第 43 轮同 sha） |

第 7 轮往后每轮都是在加功能之后重编一次。**唯一一次回退是第 36~38 轮**，
而且那三次红的是**同一个错** —— 不是新功能本身，是我漏写了一个被调用的函数
（那三次的产物是上面说的 annotation 机制，不是三次不同的 bug）。

其余每一轮新的改动都是**一次过的**：新增的屏
（22 / 26 / 27 / 15 / 16 / 06 / 21 / 04 / 30 / 08 / 09 / 10 / 37~41）、
第三个 target（分享扩展）、App 图标两次改版、两次配图相关的修复，都没有过第二次。
...
> **流水线自己也会拆包验货。** 打完 IPA 之后有一步
> 「校验 IPA 内容（扩展数量 / 版本键 / 调试动态库）」：
> **扩展必须正好 2 个**、**两个扩展的版本必须和主 App 一模一样**、
> 不能夹带调试动态库 —— 任何一样不对，这次 run 直接红，
> 不会把一个装不上的包送到你手里。
> （这三条不是摆设：run #29 / #30 已**连续两次**实测跑过并通过，产出的包都是 1.15 MB 那一档。）
...
> **失败时不用你翻日志。** 把「失败了」告诉我，我这边能直接读到这次 run 的
> **步骤级结论**（10 步里哪一步红），也能取到 job 的报错片段。
> 你也可以自己看：Actions → 那一次运行 → 点开「摘出错误与警告」那一步。
>
> 一个实测过的边界：**run 的元信息与步骤结论是公开可读的**（不用登录，能查到
> `completed / success` 和每一步的结论），但 **artifact 文件本身要登录才下得动**
> （未登录会被 `401 Requires authentication` 挡掉）。所以「绿没绿、第几步红」
> 我替你看；**要拿 IPA 还是得你在浏览器里点一下** —— 反正装包本来就在你机器上做。
...
> **照片字段三态都在**（03 新建 / 21 起手引导 / 34 编辑）。
> 这是 2026-09-14 改的：原来它外面套着 `if isEditing`，只在编辑态出现
> —— 当时的意思是「新建时先把话记下来，照片之后从详情页点『编辑』回来补」，
> 是刻意的，不是漏写。用户明确要「新建时也能直接加图」，就把那层门拆了。
>
> 拆掉之后三态共用**一张**字段表（分类 → 标题 → 图片 → 内容 → 标签 → 提醒），
> 不再是「新建少一栏」的两套 —— 他在 34 学会的位置，回到 03 依然成立。
>
> 两件顺手确认过的事：① **不需要**动「内容没写就不建记录」那条守卫 ——
> `snapshot` 里早就把 `photoHashes` 算进去了，所以「只加图、一个字没写」
> 也会正常存下来，不会点了保存却什么也没发生；② **选图那一刻记录就建出来了**：
> `onChange(of: photoHashes)` → 0.8 秒自动落盘 → 新建态那条守卫失效，
> 于是「选完图又点取消」也会留下一条记录；那张图**不是孤儿**
> （记录正引用着它，`purgeOrphans` 只清没人引用的文件）。
> ⚠️ ②原来写的是「文件会成为孤儿，由 `purgeOrphans` 收」—— 2026-09-13 复核后
> 改正：「取消」在实现里只有一句 `path.removeLast()`，它不删记录、也不删文件。
>
> ③ 由此顺出来的一件事：**顶行那颗按钮 2026-09-13 从「取消」改名「返回」**
> （03 / 08 / 21 / 34 四屏，画布 + 原型 + PNG 一起换）。理由是这一屏本来就是
> 自动保存的，状态条自己写着「已自动保存 · 刚刚」——「取消」在那儿是一句假话。
> 与「自动保存」相配的只有两颗：**离开**（返回）与**退回**（22 屏的撤销，
> 落点 `HerInfoStore.discardDraft` —— 真删、不进回收站、只配给刚新建的那一条）。
> 同一个提交里把 22 屏那颗「撤销」接上了，它此前只在设计稿里有、代码里没画。
...
> 而 CI 的 job 日志要仓库 admin 权限（返 403）、artifact 里的 `build.log`
> 下载也要认证（返 401）—— 结果是**推一次、等三分钟，才看到一行行号**。
> 于是先让 workflow 把错误 `echo "::error::…"` 发成 annotation（annotations 接口是公开可读的），
> 再把「名字找不找得到」这件事搬回本地。它只查**小写裸函数调用**与
> **五个命名空间的成员**，不查 `CGPoint(...)` 这类大写构造 —— 框架类型永远在白名单外，
> 混进来只会把真问题淹掉。**它不代替编译**：类型不匹配这类错仍然只有 CI 能发现。
...
> 补充一条已经落地的：**云端 CI 现在会把编译错误发成 annotation**。
> 换句话说，「首次编译要有人把日志拿回来」这件事，不必再等人 ——
> 每次推送后去读 `check-runs/<jobid>/annotations` 就能看到
> `文件:行:列: error: …` 的原文（最多 20 条，去重后按序）。
> 但**一次只报一批**：修掉第一条，重推，才看得见下一条。
> 所以流程上仍然是**一次修一类**，不是一次修完。
...
└── AppIcon.appiconset/
    ...
    └── AppIcon-1024.png               ← 珊瑚底 #E2857A + 银白猫 + 右下角一颗心
```
...
- **图标的造型决定 —— 这一版开始画脸了**。最初定过一条规矩「图标不画脸」，
  理由是桌面只有 60px、画五官必然糊成一团深色。**那条规矩针对的是人脸**：
  眉毛、鼻子、嘴、头发五样并存，60px 下必然互相吃掉。**猫脸只有两样必须留下
  —— 轮廓，和眼睛**。银白的身子压在珊瑚底上轮廓自己就出来；一双碧绿的眼睛
  是全图唯一的绿色，缩到 40px 仍看得出是绿的。
  所以只画到「两只眼 + 一个鼻头 + 一个 ω 嘴」，**胡须不画** ——
  它在这个尺寸上不是细节，是脏东西。
- **脸型是「一团」，不是「宽腮」**：头部渲染出来的宽高比从 1.22 收到 1.13。
  中途往 1.09 推过一版，脸当场变成「圆角方块」、不像猫了，所以退回来停在 1.13。
  换脸型时**五官的位置和尺寸一个都没动** —— 「像不像猫」这件事没有被重新下注。
- **右下角那颗心是情侣元素**：心占画布宽度的 26%，并和猫的右脸轻微交叠。
  这不是纯装饰取舍 —— 心一旦做小、贴着下巴，60px 桌面尺寸下就只剩一个点。
  四条路都试过（见「板 4」B 区）：双猫会把「她家那一只」改成「两只」，
  心形轮廓会牺牲猫的辨识度，所以选了最不牺牲主角的那条。
- **30px 是底线**：降到 30 以下这张脸不再承诺可读，图标会在「像猫」和
  「一团白」之间随机。
- **为什么是猫**：她家养的那只银渐层英短。
...
> 五种构造（正面头 / 前爪 / 项圈 / 尾巴 / 大耳）、四种底色、圆润度三档与
> 四条情侣方向的比对走查，见「我的宝宝江林桐-视觉改版稿」的板 1 C 区、板 3、板 4。
> **板 3 / 板 4 已导出为 `03-猫图标板.png` / `04-圆润与情侣板.png`**，不必开画布就能看；
> 板 3 是第一版猫（过程稿），**板 4 才是定稿**。板 2 是更早的人像版（圆环 + 马尾），
> 已停用，只作设计记录保留。回退时改前的两版都在该目录：
> `图标-猫-圆润前（第一版猫）.png`、`图标-旧版-圆环与马尾.png`。
> 主色 `#E2857A`（暖粉珊瑚）。完整色板与图标规格见该目录 ——
> **该目录里有一份 `图标清单.md`**，逐文件标注了定稿 / 备选 / 对照 / 可回退，
> 不必靠文件名猜。**全目录只有一张带「主推」二字，就是定稿那道。**
