...
| **08 键盘附件栏** | 画布上键盘上方那排「分类・喜好 / 标签 / 提醒 / 记录时间」胶囊。<br>**底部按钮不被键盘挡住这件事已经是好的**（`EditorView` 的保存条在 `ScrollView` 下方，<br>SwiftUI 的键盘避让会把它顶上去）—— 缺的只是那排「不用收键盘就能换字段」的快捷入口 |

**30 搜索筛选面板**（`SearchFilterView.swift`）与 **07 历史版本对比**（`VersionHistoryView.swift`）
这一轮做掉了。
