...
| 第 7–11 轮 | **全部成功** | 预检清单第 6 节 · 引导屏 + 轻提示 + 删除二次确认 + 回收站 + 首条记录引导 · 推送脚本 v4/v5。**从第 4 轮起连续 8 次全绿** |

第 7 轮往后每轮都是在加功能之后重编一次，**没有一次回退** ——
新增的屏（22 / 26 / 27 / 15 / 16 / 06 / 21）都是一次过的。
...
一共 21 个 Swift 源文件（主 App 17 + 扩展 3 + 两者共用的契约 1），
...
盘一遍库存之后，**真正还剩的只有 3 项** —— 而且只有第一项不是「重复劳动」：

| 还剩 | 性质 |
|---|---|
| **09 / 10 情绪打标** | **要新建两个 target**（App Intent + Share Extension），不是加个 View 的事 |
| **30 搜索筛选面板** | 一次性草稿态、不落库；照 `SecondaryViews.swift` 里 27 的写法搬 |
| **08 键盘态** | 已被「搜索独立成屏」的设计**规避**掉了（`HomeView.swift:208` 的注释），输入框的聚焦态在 `HITextField` 里。<br>没有独立屏，也不需要有 |
...
> **07 历史版本对比**也做掉了（`VersionHistoryView.swift`）。它比其余几屏多一层判断：
