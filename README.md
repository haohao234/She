...
代码在 `main` 上，云端 macOS 编译**第 4 轮起连续 29 次成功**（run 4~32），
...
| 第 7–32 轮 | **全部成功** | 预检清单第 6 节 · 引导屏 + 轻提示 + 删除二次确认 + 回收站 + 首条记录引导 · 推送脚本 v4/v5 · **07 历史版本左右对比** · **04/30 搜索与筛选** · **08 键盘工具栏** · **09/10 情绪打标与即时建议** · **09 系统级入口（长按图标 / 快捷指令）** · **分享扩展** · **可侧载 IPA** · **Release 打包 + 拆包自检（扩展数量 / 版本键 / 调试动态库）** · **真机安装成功**。<br>**从第 4 轮起没有一次回退** —— 截至 `6992d38`（第 32 轮）已连续 **29 次成功**（按 CI 实际运行次数计）。<br>但**「成功」不等于「干净」**：第 12~25 轮（连续 14 轮）每一轮都带着同一条 warning 而没人发现，现在它是会红的断言 |
